...
.then((res)=>{
    ...
    const checkFilter = res.events.map(event => event.category).filter((category, index, array) => array.indexOf(category) === index)
    ...
    let returnValueSearch = () => {
        let cardsFilter = res.events.filter(event => event.name.toLowerCase().startsWith(searchContainer.value.toLowerCase()))
